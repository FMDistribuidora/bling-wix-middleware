// index.js - CORRIGIDO v4.2 JWT READY (exibe refresh_token completo no /callback)
    ...
    try {
      ...
      const response = await axios({
        ...
        headers: {
          ...
          'enable-jwt': '1' // Header obrigatório para novo padrão JWT
        },
        timeout: 15000,
        validateStatus: () => true
      });
...
app.get('/', (req, res) => {
  res.send(`
    ...
    <h2>✅ Sistema Online - VERSÃO CORRIGIDA v4.2 JWT READY</h2>
    <ul>
...
app.get('/produtos', async (req, res) => {
  try {
    ...
    if (cacheValido()) {
      ...
      return res.json({
        sucesso: true, produtos: produtosCache, total: produtosCache.length, fonte: 'cache', versao: 'CORRIGIDA v4.2 JWT READY',
    ...
    if (fetchInProgress && fetchPromise) {
      ...
      try {
        ...
        return res.json({ sucesso: true, produtos, total: produtos.length, fonte: 'concurrent_fetch_result', versao: 'CORRIGIDA v4.2 JWT READY' });
      } catch (err) {
        if (produtosCache.length > 0) {
          return res.json({ sucesso: true, produtos: produtosCache, total: produtosCache.length, fonte: 'cache_fallback', versao: 'CORRIGIDA v4.2 JWT READY' });
        }
        throw err;
      }
    }
    ...
    res.json({
      sucesso: true, produtos, total: produtos.length, fonte: 'bling_direto', versao: 'CORRIGIDA v4.2 JWT READY',
    ...
    if (produtosCache.length > 0) {
      return res.json({ sucesso: true, produtos: produtosCache, total: produtosCache.length, fonte: 'cache_fallback', versao: 'CORRIGIDA v4.2 JWT READY', aviso: 'Dados do cache devido a erro na API' });
    }

    res.status(500).json({ erro: error.message, produtos: [], total: 0, versao: 'CORRIGIDA v4.2 JWT READY', timestamp: new Date().toISOString() });
  }
});

app.get('/ping', (req, res) => {
  res.json({ status: 'alive', versao: 'CORRIGIDA v4.2 JWT READY', timestamp: new Date().toISOString(), cache: { produtos: produtosCache.length, valido: cacheValido(), timestamp: cacheTimestamp ? new Date(cacheTimestamp).toISOString() : null } });
});

app.listen(PORT, async () => {
  console.log(`🚀 Servidor CORRIGIDO rodando na porta ${PORT}`);
  console.log(`🎉 VERSÃO: CORRIGIDA v4.2 JWT READY`);
  if (PRELOAD_CACHE) {
